...
// routes. The effect below still re-fetches on mount/focus/interval so the
// CMS admin's live preview keeps working after that first paint.
export function useIndustriesPage(initialData = null) {
  const [data, setData] = useState(initialData);
  useEffect(() => {
    let active = true;
    const load = () => getIndustriesPage().then((next) => { if (active) setData(next); }).catch(() => {});
    const onFocus = () => load();
    load();
