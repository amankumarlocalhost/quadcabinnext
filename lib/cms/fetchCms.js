...
export async function fetchCmsPage(slug, { fresh = false } = {}) {
  ...
  // refresh()) — bypasses the 60s ISR cache entirely so admin edits show up
  // on the next poll instead of waiting out the server-side revalidation
  // window. Server-rendered page.js calls keep the default (cached, fast
  // first paint) since they don't pass this.
